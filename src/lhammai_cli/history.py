...
class ConversationHistory:
    ...
    @staticmethod
    def load_history_from_disk() -> dict[str, Conversation]:
        ...
        try:
            ...
            logger.error(f"History file not found: {e}")
            raise
    ...
    def load_from_disk(cls, uuid: UUID) -> "ConversationHistory":
        ...
        if uuid not in history:
            raise FileNotFoundError(f"Conversation {uuid} not found in history")
    ...
    def add_message(self, role: Role, content: str) -> None:
        ...
        if not self._current_conversation:
            raise RuntimeError("No conversation started. Call start_new_conversation() first.")
    ...
    def get_current_uuid(self) -> str | None:
        ...
        """
        with self._lock:
            return str(self._current_uuid) if self._current_uuid else None
    ...
    def list_conversation_uuids(self) -> list[str]:
        """List all conversation UUIDs.

        Returns:
            List of conversation UUID strings
        """
        history = self.load_history_from_disk()
        return list(history.keys())

    def get_conversation_by_uuid(self, conversation_uuid: str) -> list[dict[str, str]] | None:
        """Get a specific conversation by its UUID.

        Args:
            conversation_uuid: UUID of the conversation to retrieve
        ...
        history = self.load_history_from_disk()
        conversation = history.get(conversation_uuid)
        return [msg.model_dump() for msg in conversation.messages] if conversation else None

    def save_to_disk(self) -> None:
        """Save the current conversation to disk."""
        with self._lock:
            if not self._current_conversation or not self._current_uuid:
